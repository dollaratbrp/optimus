...
class FastLoadsBox(VerticalScrolledFrame):
    ...
    def get_complete_dataframe(skus_list, qty_list, crate_type):
        ...
        if crate_type == 'W':
            ...
            LEFT JOIN (SELECT *
            FROM  [MASTERDATA].[DBO].[MD_MATERIAL_SAP_CLASSIFICATION_VIEW] AS CLASSIF
            WHERE CLASS = 'RETURNABLE_CRATE' AND LANGUAGE_CODE = 'E' AND CHARACTERISTIC_NAME = 'VEHIC_LENGHT_LEG2') AS L
            ON SKID.MATERIAL_NUMBER = L.MATERIAL_NUMBER LEFT JOIN (SELECT *
            FROM  [MASTERDATA].[DBO].[MD_MATERIAL_SAP_CLASSIFICATION_VIEW] AS CLASSIF
            WHERE CLASS = 'RETURNABLE_CRATE' AND LANGUAGE_CODE = 'E' AND CHARACTERISTIC_NAME = 'VEHIC_WIDTH_LEG2') AS W
            ON SKID.MATERIAL_NUMBER = W.MATERIAL_NUMBER LEFT JOIN (SELECT *
            FROM  [MASTERDATA].[DBO].[MD_MATERIAL_SAP_CLASSIFICATION_VIEW] AS CLASSIF
            WHERE CLASS = 'RETURNABLE_CRATE' AND LANGUAGE_CODE = 'E' AND CHARACTERISTIC_NAME = 'MIN_REQUIRED_LEG2') AS H
